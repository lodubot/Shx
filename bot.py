...
async def start(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    await update.message.reply_text(
        ...
        f"🔗 Termius/Termux SSH connection link ke liye: `/link`\n"
...
async def get_link(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    await update.message.reply_text("Tmate SSH session ban raha hai, 4 seconds wait karein...")
    ...
    try:
        ...
        # Tmate ko background mein run karna (-F foreground mode mein chalakar nohup se log mein dalna)
...
async def delete_vps(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    try:
        # User ka tmate process kill karna
        subprocess.run(f"pkill -f 'tmate.*user_data_{chat_id}'", shell=True)
        ...
        await update.message.reply_text("❌ Aapka tmate session aur SSH connection terminate kar diya gaya hai.")
    except Exception as e:
        await update.message.reply_text(f"Error: {e}")
